...
*Generated automatically based on actual workspace movement over the past 24 hours.*`;
  ...
  const generateTaskBreakdown = async (featureName) => {
    ...
    // Return structured list of new tasks that can be created
    return [
      {
  ...
  const reviewCode = async (code, language) => {
    ...
    // Dynamic, smart code parsing to give highly realistic feedback
    let bugs = [];
    let performance = [];
    let security = [];
    ...
    // Language-specific and general code heuristics
    ...
    // Default suggestions if code is clean
